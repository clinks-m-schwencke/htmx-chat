(() => {
  ...
  function createWebSocket(url, entry) {
    ...
    try {
      ...
      entry.socket.addEventListener("open", () => {
        ...
        if (firstElement) {
          triggerEvent(firstElement, "htmx:after:ws:connect", {
            url,
            socket: entry.socket,
          });
        }
  ...
  function sendMessage(element, event) {
    ...
    if (!url) {
      ...
      let ancestor = element.closest(
        "[" + prefix + "hx-ws\\:connect],[" + prefix + "hx-ws-connect]",
      );
      if (ancestor) {
    ...
    if (!url) {
      console.error(
        "No WebSocket connection found for hx-ws:send element",
        element,
      );
      return;
    }
  ...
  function handleMessage(entry, event) {
    ...
    try {
      ...
      let firstElement = entry.elements.values().next().value;
      if (firstElement) {
        triggerEvent(firstElement, "htmx:wsUnknownMessage", {
          data: event.data,
        });
      }
      return;
    ...
    if (
      !triggerEvent(targetElement, "htmx:before:ws:message", {
        envelope,
    ...
    if (envelope.channel === "ui" && envelope.format === "html") {
      ...
      (envelope.channel === "audio" ||
        ...
        envelope.channel === "binary")
    ) {
      // Known custom channel - emit event for extensions to handle
      triggerEvent(targetElement, "htmx:wsMessage", {
        ...envelope,
        element: targetElement,
      });
    } else {
      // Unknown channel/format - emit unknown message event
      triggerEvent(targetElement, "htmx:wsUnknownMessage", {
        ...envelope,
        element: targetElement,
      });
    }

    triggerEvent(targetElement, "htmx:after:ws:message", {
      envelope,
      element: targetElement,
    });
  }
  ...
  function swapContent(target, content, sourceElement, envelopeSwap) {
    let swapStyle =
      envelopeSwap ||
      api.attributeValue(sourceElement, "hx-swap") ||
  ...
  function initializeSendElement(element) {
    ...
    if (!triggerSpec) {
      ...
      triggerSpec = element.matches("form")
        ? "submit"
        : element.matches("input:not([type=button]),select,textarea")
          ? "change"
          : "click";
    }
  ...
  function cleanupElement(element) {
    ...
    if (element._htmx?.wsSendHandler) {
      element.removeEventListener(
        element._htmx.wsSendEvent,
        element._htmx.wsSendHandler,
      );
    }
  }
  ...
  function checkLegacyAttributes(element) {
    ...
    if (element.hasAttribute("ws-connect") || element.hasAttribute("ws-send")) {
      console.warn(
        "HTMX WebSocket: Legacy attributes ws-connect and ws-send are deprecated. Please use hx-ws:connect/hx-ws-connect and hx-ws:send/hx-ws-send instead.",
      );
  ...
  htmx.registerExtension("ws", {
    ...
    htmx_after_process: (element) => {
      ...
      element
        .querySelectorAll(
          "[hx-ws\\:connect], [hx-ws-connect], [hx-ws\\:send], [hx-ws-send], [hx-ws], [ws-connect], [ws-send]",
        )
        .forEach(processNode);
    },
